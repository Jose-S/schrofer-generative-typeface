...
def drawGrid(width = 1):
    ...
    line((part, 0),(part, dimension))
    line((part*2, 0),(part*2, dimension))
    line((0, part),(dimension, part))
    line((0, part*2),(dimension, part*2))
...
def drawColoredGlyph(g, width = 3, innerLines = 7, vColor=(1,0,0), hColor=(.95,0,0)):
...
def drawLabels(n, amount=9):
    txt = FormattedString(font="Helvetica", fontSize=12)
    txtU = FormattedString(font="Helvetica", fontSize=12)
    page = FormattedString(f"{n+1}",font="Helvetica", fontSize=12)
    ...
    # position
    anchor = 32
    text(page, (width()-anchor, height()-anchor))
    text(txt, (anchor, height()-anchor))
    text(txtU, (anchor, height()-anchor*2.5))
...
def drawLabelsLand(n, amount=9):
    txt = FormattedString(font="Helvetica", fontSize=12)
    txtU = FormattedString(font="Helvetica", fontSize=12)
    page = FormattedString(f"{(n+1)*2}",font="Helvetica", fontSize=12)
    ...
    # position
    anchor = 32
    text(page, (width()-anchor*2, anchor))
    text(txt, (width()-anchor*4, height()-anchor))
    text(txtU, (width()-anchor*4, height()-anchor*2.5))
        ...
        for j in range(2):
            ...
            for r in range(0, dim):
                for c in range (0, dim):
                    ...
                    if(j == 1):
                        drawGlyph(graphs[i - sub], .25, innerLines=5)
                        sub -= 1
...
path = "/Users/josesaravia/Projects/Typography/Graph/glyph_design_space.pdf" 
saveImage(path)                   
openPreview(path)
